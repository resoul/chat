...
export default {
  ...
  mounted() {
    ...
    const rightSidebarTab = new Tab(this.$refs.tab, (id) => {
      header.forEach((node) => {
    ...
    header.forEach((node) => {
      if (node.dataset.header !== rightSidebarTab.aciveTab)
...
<template>
  <div class="drawer drawer-right">
    <div :class="{ hidden : !sidebar.hasOpen }" class="drawer-overlay fixed inset-0 z-[150] bg-slate-900/60"></div>
    <div :class="{ hidden : !sidebar.hasOpen }" class="drawer-content fixed right-0 top-0 z-[151] h-full w-full sm:w-80">
      <div ref="tab" class="right-sidebar-tab-wrapper w-ful relative flex h-full flex-col bg-white dark:bg-navy-750">
        <div class="flex items-center justify-between py-2 px-4">
          ...
          <button @click="sidebar.close()" class="btn -mr-1 size-6 rounded-full p-0 hover:bg-slate-300/20 focus:bg-slate-300/20 active:bg-slate-300/25 dark:hover:bg-navy-300/20 dark:focus:bg-navy-300/20 dark:active:bg-navy-300/25">
            <svg
                xmlns="http://www.w3.org/2000/svg"
                class="size-4"
                fill="none"
